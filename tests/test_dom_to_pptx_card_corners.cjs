  ...
  try {
    ...
    const classification=await page.evaluate(()=>{
      const cases=[
        ...
        {name:'native backdrop surface',style:'backdrop-filter:blur(3px)',ignored:true},
        {name:'unsupported backdrop remains',style:'background:transparent;backdrop-filter:blur(3px)',reason:'backdrop-filter'},
