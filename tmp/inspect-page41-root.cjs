...
(async()=>{
 ...
 const page=await b.newPage({viewport:{width:1280,height:720}});await page.setContent(html);
 console.log(await page.evaluate(()=>[...document.body.children].map(e=>({tag:e.tagName,cls:String(e.className),html:e.outerHTML.slice(0,120)}))));
 await b.close();
})();
